...
def test_falkner_skan_x(a0, dstar0, H0, dt, nsteps, closure):
    nu, x0 = 1E-4, 0.4
    nx, nz = 100, 1
    ...
    def extend_trQ(Q):
        Q_ext = zeros([Q.shape[0]+2, Q.shape[1]+2]) + Q0
        Q_ext[1:-1,1:-1] = Q
        Q_ext[1:-1,0] = Q[:,0]
        Q_ext[1:-1,-1] = Q[:,-1]
        Q_ext[-1,:] = Q_ext[-2,:]
    ...
    ibl = IBL(xgrid, zgrid, nu, dt, closure,
              extend_M=extend_M, extend_trQ=extend_trQ)
    x = ibl.settings.xc[:,0]
    Z = zeros([nx,nz])
    ...
    ibl.init(array([M0 * 3 + Z, Z]), Q0 * 3 + Z)
...
a_dstar_H_dt_nsteps_table = array([[2, 0.47648, 2.1882, 1E-3, 10],
                                   ...
                                   [-0.08, 2.67173, 3.22, 8E-3, 20],
                                   [-0.09043, 3.49786, 4.02916, 1E-2, 20],
                                   [-0.087, 4.14726, 4.87975, 1E-2, 20],
                                   [-0.08, 4.7554, 5.89021, 1E-2, 20]])

if __name__ == '__main__':
    for a0, dstar0, H0, dt, nsteps in a_dstar_H_dt_nsteps_table[:7]:
        figure()
        test_falkner_skan_x(a0, dstar0, H0, dt, nsteps, ExtendedLaminar2dClosure)
        title('H={}'.format(H0))
    for a0, dstar0, H0, dt, nsteps in a_dstar_H_dt_nsteps_table[:8]:
        figure()
        test_falkner_skan_x(a0, dstar0, H0, dt, nsteps, ClassicLaminarSteady2dClosure)
        title('H={}'.format(H0))
    for a0, dstar0, H0, dt, nsteps in a_dstar_H_dt_nsteps_table[:7]:
        figure()
        test_falkner_skan_x(a0, dstar0, H0, dt, nsteps, ClassicLaminar2dClosure)
        title('H={}'.format(H0))
